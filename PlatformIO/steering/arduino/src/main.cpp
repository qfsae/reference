...
#define CAN_OE 10     // transceiver enable
...
bool CANInit(BITRATE bitrate, int remap)
{
  ...
  //Serial2.print("can2=");
  //Serial2.println(can2);
  if (can2)
  {
    Serial2.println("CAN2 initialize ok");
  }
  else
  {
    Serial2.println("CAN2 initialize fail!!");
  }
  ...
  //Serial2.print("can1=");
  //Serial2.println(can1);
  if (can1)
  {
    Serial2.println("CAN1 initialize ok");
  }
  else
  {
    Serial2.println("CAN1 initialize fail!!");
    return false;
  }
  return true;
}
...
void loop()
{
  ...
  if (CANMsgAvail(recv_ch))
  {
    ...
    if (CAN_RX_msg.type == DATA_FRAME)
    {
      ...
      {
        // Serial2.print("0x");
        Serial2.write(CAN_RX_msg.data[i]);
